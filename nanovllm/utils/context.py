...
class Context:
    ...
    # identical. The attention layer reads the prefix's K/V once with a
    # non-causal varlen pass, attends over each per-seq unique tail with a
    # second varlen pass, then merges the two outputs via LSE composition.
    # Cuts per-step KV-cache HBM traffic from O(N·L) to O(L + N·tail) when
    # there's a long shared prefix.
    ...
    tail_max_len: int = 0                             # allocated tail token span used as the FA varlen launch bound
    ...
    cascade_wrapper: object | None = None             # optional FlashInfer planned wrapper
...
def set_context(is_prefill, cu_seqlens_q=None, cu_seqlens_k=None, max_seqlen_q=0, max_seqlen_k=0, slot_mapping=None, context_lens=None, block_tables=None,
                ...
                cu_q_pref=None, cu_k_pref=None, cu_q_suff=None, cu_k_suff=None, cascade_wrapper=None):
    global _CONTEXT
    _CONTEXT = Context(is_prefill, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, slot_mapping, context_lens, block_tables,
                      ...
                      cu_q_pref, cu_k_pref, cu_q_suff, cu_k_suff, cascade_wrapper)
