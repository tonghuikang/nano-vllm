...
def request_body(model, prompt_ids, output_tokens):
    return json.dumps({
        "model": model,
        "prompt": prompt_ids,
...
def one_body(url, body, api_key, timeout_s):
...
def one(url, model, prompt_ids, output_tokens, api_key, timeout_s):
    body = request_body(model, prompt_ids, output_tokens)
    return one_body(url, body, api_key, timeout_s)
...
def sweep_cell(url, model, N, prefix_tokens, seed_base, api_key, timeout_s):
    shared_prefix = random_token_ids(prefix_tokens, seed=seed_base)
    out_tok = output_tokens_for(N)
    body = request_body(model, shared_prefix, out_tok)
    t0 = time.perf_counter()
    with cf.ThreadPoolExecutor(max_workers=N) as ex:
        futs = [ex.submit(one_body, url, body, api_key, timeout_s) for _ in range(N)]
...
def cold_prefill(url, model, prefix_tokens, seed, api_key, timeout_s):
    ...
    dt, _, _ = one(url, model, prompt, output_tokens=1, api_key=api_key, timeout_s=timeout_s)
    return dt


def warmup(url, model, api_key, max_concurrency, timeout_s):
    Nw = min(8, max_concurrency)
    shared_prefix = random_token_ids(64, seed=99_000)
    body = request_body(model, shared_prefix, 16)
    with cf.ThreadPoolExecutor(max_workers=Nw) as ex:
        list(ex.map(lambda _: one_body(url, body, api_key, timeout_s), range(Nw)))
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--timeout-s",
        type=float,
        default=float(os.environ.get("BENCH_TIMEOUT_S", "600")),
    ...
    print(
        ...
        f"concurrencies={args.concurrencies} timeout_s={args.timeout_s:g}",
        file=sys.stderr,
    )
    print("# Warming up engine…", file=sys.stderr)
    warmup(args.url, args.model, args.api_key, max(args.concurrencies), args.timeout_s)
    ...
    for L in args.prefix_lengths:
        prefill_s = cold_prefill(
            args.url, args.model, L, seed=seed_base + 50_000,
        ...
        for N in args.concurrencies:
            try:
                tps, n_req, p_tok, c_tok, wall = sweep_cell(
                    args.url, args.model, N, L, seed_base, args.api_key, args.timeout_s
                )
